 ...
 * than of this renderer (WIRE_FORMAT.md 3.6.24): a `staticRows` grid honours no
 * row action in any tier, so its rows carry no marker whatever the grid
 * declares.
 *
 * The leading space matches `gridPrintBreakClasses` above: the caller appends
 * the fragment to a base class string.
 */
export const gridRowInteractiveClass = (hasRowAction: boolean): string =>
  hasRowAction ? ' fuaran-grid-row-interactive' : '';
