...
// The same corpus root the script resolves: FUARAN_WIRE_FIXTURES when set (a
// worktree naming its own corpus checkout — the script refuses a bad value, so
// this guard then runs and reports that refusal), else the canonical sibling.
const authority =
  process.env['FUARAN_WIRE_FIXTURES'] ||
  join(packageRoot, '..', '..', '..', 'wire-format-fixtures');

describe.skipIf(
  !process.env['FUARAN_WIRE_FIXTURES'] && !existsSync(join(authority, 'manifest.json')),
