...
**`relay@1.5` — the `hatches` read (Phase 1842), additive.** The peer advances `relay@1.3` → `relay@1.5`: it serves the `hatches` request type and its capability of the same name (`DEVTOOLS_RELAY.md` §7.8), answering with the runtime section of the escape-hatch report, and advertises it only at a session of `relay@1.5` or later, over a surface that has the report. `relay@1.4` needed nothing here: its additions are a declaration a page peer makes by omitting it and a refusal only an upstream-tree peer raises, and this peer's tree is in the page by construction. `FuaranDebugGlobal` gains an OPTIONAL `hatches()` (so a hand-built surface still type-checks; `DEBUG_GLOBAL_VERSION` 0.3.0), `DebugGlobalOptions` gains optional `customRenderers` / `customHashFloor` (what the report is handed — an input not handed is reported `undecided`, never `closed`), `CustomRendererRegistry` gains `registrations()`, and the report's producer is exported as `observeRuntimeHatches`. No existing exchange or member changed. **The number does not move**: this rides the next `@fuaran-ui/renderer` release.
